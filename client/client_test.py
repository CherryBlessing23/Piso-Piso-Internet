...
def check_network_connection():
    while True:
        try:
            requests.get(f'http://{serverIP}', timeout=10)
...
def handle_coin_slot_check(is_busy, id_input_time_remaining):
    global user_idnumber, serverIP, logged_idnumber, client_id, id_input_window
    id_input_time_remaining = tk.Toplevel()
    id_input_time_remaining.attributes('-topmost', True)
    ...
    if client_id is not None:
        try:
            response = requests.get(f"http://{serverIP}/get_rates_guest")
            response.raise_for_status()
